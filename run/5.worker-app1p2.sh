...
if [ ! -z $1 ]; then 
  ...
  if (($max_num == 1)); then
    ...
    max_argu="--precommit1max=$task_num --precommit2max=3"
  fi
fi
...
if [ $pid -le 0 ]; then
  ...
  # log
  export marklog="worker-$ENV_SECTOR_SIZE-app1p2.log"
  # backup log
  if [ -f "$marklog" ]; then 
  ...
  # tips
  echo -e "\033[34m nohup ${EXE_LOTUS_WORKER} run --listen=$localip:$ENV_LOTUS_WORKER_PORT --commit=false $max_argu $huge_argu > $marklog 2>&1 & \033[0m"

  nohup ${EXE_LOTUS_WORKER} run --listen=$localip:$ENV_LOTUS_WORKER_PORT --commit=false $max_argu $huge_argu > $marklog 2>&1 &
else
  # tips
