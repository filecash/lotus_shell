...
if [ $pid -le 0 ]; then
  ...
  if [ $gpu_num -eq 0 ]; then
    ...
    export RUST_FIC_OPTION_GPU_ID=$gpu_argu
  fi

  # log
  export marklog="worker-$ENV_SECTOR_SIZE$nogpu-c.log"
  # backup log
  if [ -f "$marklog" ]; then 
  ...
  # tips
  echo -e "\033[34m nohup ${EXE_LOTUS_WORKER} run --listen=$localip:$ENV_LOTUS_WORKER_PORT --addpiece=false --precommit1=false --precommit2=false $max_argu $huge_argu > $marklog 2>&1 & \033[0m"

  nohup ${EXE_LOTUS_WORKER} run --listen=$localip:$ENV_LOTUS_WORKER_PORT --addpiece=false --precommit1=false --precommit2=false $max_argu $huge_argu > $marklog 2>&1 &
else
  # tips
