...
if [ $pid -le 0 ]; then
  ...
  # tips
  echo -e "\033[34m nohup ${EXE_LOTUS_WORKER} run --listen=$localip:$ENV_LOTUS_WORKER_PORT --addpiece=false --precommit2=false --commit=false $max_argu $huge_argu > $marklog 2>&1 & \033[0m"

  nohup ${EXE_LOTUS_WORKER} run --listen=$localip:$ENV_LOTUS_WORKER_PORT --addpiece=false --precommit2=false --commit=false $max_argu $huge_argu > $marklog 2>&1 &
else
  # tips
